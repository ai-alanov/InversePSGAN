...
class InversePSGAN2(PSGAN):
    ...
    def _build_network(self):
        self.X2 = lasagne.layers.InputLayer((None, self.config.nc,
                                             self.config.npx, self.config.npx))
        self.bin_rand = T.scalar('bin_rand', dtype='int32')
        self.Z_loc_and_period = lasagne.layers.SliceLayer(
            self.Z, indices=slice(self.config.nz_global, None), axis=1)
    ...
    def _build_obj(self):
        ...
        self.entropy = gen_Z_out_1 - gen_Z_out_2 + 10.
        self.entropy = T.sum(self.entropy ** 2, -1)
        self.entropy = T.mean(-self.entropy) # T.mean(T.exp(-self.entropy)) # T.mean(1. / self.entropy)
        if self.use_entropy:
...
class InversePSGAN3(PSGAN):
    ...
    def __init__(self, name=None, compile=True, **kwargs):
        super(InversePSGAN3, self).__init__(name, **kwargs)
